package validators.users.abstracts;

import entities.users.abstracts.User;
import exceptions.validators.UserValidationException;
import validators.users.interfaces.UserValidatorInterface;

public abstract class UserValidatorAbstract<T extends User> implements UserValidatorInterface<T> {
    ...
    public void validateId(long id) throws UserValidationException{
        ...
        if(!Sid.endsWith("00") || !Sid.endsWith("01")){
            throw new UserValidationException("Invalid ID for a user");
        }
