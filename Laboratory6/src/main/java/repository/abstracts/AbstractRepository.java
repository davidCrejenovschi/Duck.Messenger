...
public abstract class AbstractRepository<T> implements Repository<T> {
    ...
    protected Collection<T> getByIdsFromCache(List<Long> ids){
    ...
    @Override
    public abstract void exists(long id) throws RepositoryException;
    protected void existInCache(long id) {
        if (!items.containsKey(id)) {
            throw new RepositoryException("Item with id " + id + " was not found.");
        }
    }
