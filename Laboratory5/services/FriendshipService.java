...
public class FriendshipService {
    ...
    AbstractMemoryBridgeRepository<Friendship> repository;
    FriendshipValidator validator;

    public FriendshipService(AbstractMemoryBridgeRepository<Friendship> repository_init, FriendshipValidator validator_init) {
    ...
    public void addFriendship(Long leftId, Long rightId) throws FriendshipValidationException {
        Friendship friendship = new Friendship(leftId, rightId);
        validator.validateFriendship(friendship);
    ...
    public Friendship getFriendship(long id) throws FriendshipValidationException {
    ...
    public int viewCommunities(Collection<Friendship> friendships) {
        Map<Long, Set<Long>> graph = getFriendshipGraph(friendships);
        return countConnectedComponents(graph);
    }
    private Map<Long, Set<Long>> getFriendshipGraph(Collection<Friendship> friendships) {

        Map<Long, Set<Long>> graph = new HashMap<>();

        for (Friendship pair : friendships) {
    ...
    public Set<Long> viewMostSociableCommunity(Collection<Friendship> friendships) {
